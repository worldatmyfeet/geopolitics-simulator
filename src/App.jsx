...
const G = `
  ...
  .outlet-card:hover{border-color:#a09880!important;}
`;
...
function StatPill({ label, value, prev }) {
  ...
  return (
    <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"10px 8px",textAlign:"center"}}>
      ...
      <div style={{fontSize:17,fontWeight:500,color,lineHeight:1,marginBottom:delta!==0?2:0}}>{value}</div>
      {delta !== 0 && <div style={{fontSize:10,color:delta>0?"#22c55e":"#ef4444",fontWeight:500}}>{delta>0?"+":""}{delta}</div>}
    </div>
  );
}
...
// ── NEWSPAPER PANEL (inline, not modal) ───────────────────────────────────────
  ...
  return (
    <div style={{background:"#faf8f2",border:"0.5px solid #d0c8b0",borderRadius:"var(--border-radius-lg)",overflow:"hidden",animation:"fadeUp 0.4s ease"}}>
      ...
      <div style={{padding:"12px 16px 14px"}}>
        ...
        <h2 style={{fontFamily:"Georgia,serif",fontSize:18,fontWeight:700,lineHeight:1.25,color:"#1a1a1a",margin:"0 0 6px"}}>
          {paper.headline}
        </h2>
        {paper.subHeadline && (
        ...
        <div style={{display:"grid",gridTemplateColumns: paper.pullQuote ? "1fr 130px" : "1fr",gap:12,marginBottom:10}}>
          <p style={{fontFamily:"Georgia,serif",fontSize:12,lineHeight:1.7,color:"#222",margin:0}}>
            {paper.body}
          </p>
          {paper.pullQuote && (
        ...
        {paper.opEd && (
          <>
            ...
            <div style={{background:"#f0ece0",borderRadius:4,padding:"8px 12px",display:"flex",gap:10,alignItems:"flex-start"}}>
              <div style={{width:30,height:30,borderRadius:"50%",background:outlet.accentColor,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>
        ...
        {paper.ticker?.length > 0 && (
          <div style={{background:"#1c1c1c",margin:"12px -16px -14px",padding:"5px 12px",display:"flex",gap:0,overflowX:"hidden"}}>
            ...
            <span style={{fontSize:9,color:"#bbb",fontFamily:"Georgia,serif",letterSpacing:"0.04em",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
              {paper.ticker.join("  ·  ")}
            </span>
          </div>
        )}
  ...
  return (
    <div style={{background:"#faf8f2",border:"0.5px solid #d0c8b0",borderRadius:"var(--border-radius-lg)",padding:"16px",animation:"fadeUp 0.3s ease"}}>
      ...
      {[80,55,100,40,70].map((w,i)=>(
        <div key={i} style={{height:i===0?18:i===2?11:10,width:`${w}%`,background:"#e0d8c8",borderRadius:2,marginBottom:8,animation:"pulse 1.5s ease infinite",animationDelay:`${i*0.15}s`}}/>
      ))}
    </div>
  );
}
...
function StatDeltaPanel({ stats, prevStats }) {
  ...
  return (
    <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",animation:"fadeUp 0.4s ease 0.2s both"}}>
    ...
    <div style={{fontFamily:"var(--font-sans)",maxWidth:900,margin:"0 auto",padding:"1.5rem 1rem"}}>
      ...
      {showApiInput ? (
        <div style={{background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-secondary)",borderRadius:"var(--border-radius-lg)",padding:"1rem",marginBottom:"1.25rem"}}>
          <p style={{fontSize:12,color:"var(--color-text-secondary)",margin:"0 0 8px"}}>Enter your API key to power the AI simulation:</p>
        ...
        <button onClick={()=>setShowApiInput(true)} style={{fontSize:11,color:"var(--color-text-tertiary)",background:"none",border:"none",cursor:"pointer",padding:"0 0 1.25rem",textDecoration:"underline"}}>
          {apiKey?"✓ API key set — change":"Set API key (required to play)"}
        </button>
      )}
      ...
      {/* Country grid — clean, no clutter */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
        {filtered.map(c=>(
          <button key={c.id} onClick={()=>onSelect(c)} className="country-btn"
            style={{textAlign:"left",padding:"14px 14px",borderRadius:"var(--border-radius-lg)",border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-primary)",cursor:"pointer",fontFamily:"var(--font-sans)",transition:"border-color 0.15s",width:"100%"}}>
...
function ScenarioSelectScreen({ country, onSelect, onBack, loading }) {
  return (
    <div style={{fontFamily:"var(--font-sans)",maxWidth:860,margin:"0 auto",padding:"1.5rem 1rem"}}>
      <style>{G}</style>
      <button onClick={onBack} style={{fontSize:12,color:"var(--color-text-secondary)",background:"none",border:"none",cursor:"pointer",padding:"0 0 1rem",display:"flex",alignItems:"center",gap:5}}>← Back</button>
...
export default function GeopoliticsSimulator() {
  ...
  const [gameState, setGameState]           = useState("choosing"); // choosing | reacting | reading_press | done
  const [reactionText, setReactionText]     = useState("");
  const [statDeltas, setStatDeltas]         = useState(null);   // prevStats snapshot for delta display
  const [randomEvent, setRandomEvent]       = useState(null);
  const [pendingNext, setPendingNext]       = useState(null);   // buffered next-turn data
  const [loading, setLoading]               = useState(false);
  const [newspaper, setNewspaper]           = useState(null);   // current edition
  const [allEditions, setAllEditions]       = useState([]);
  const [newspaperLoading, setNewspaperLoading] = useState(false);
  const [log, setLog]                       = useState([]);
  const [apiKey, setApiKey]                 = useState("");
  const [showApiInput, setShowApiInput]     = useState(false);
  const [sideTab, setSideTab]               = useState("stats"); // stats | alliances | actors
  ...
  async function startGame(c, s) {
    ...
    setNewspaper(null); setAllEditions([]); setStatDeltas(null); setGameState("choosing");
    const initRel={};
...
{"situation":"2-3 vivid sentences on the crisis from ${c.name}'s perspective, referencing specific neighbours or actors.","internalEvent":"One sentence about immediate internal pressure.","actions":[{"id":"a1","label":"Short label","description":"One tradeoff sentence","statHints":{"Economy":0,"Military":5,"Diplomacy":-5,"Stability":-3,"GlobalPrestige":2}},{"id":"a2","label":"Short label","description":"One tradeoff sentence","statHints":{"Economy":-3,"Military":0,"Diplomacy":8,"Stability":2,"GlobalPrestige":5}},{"id":"a3","label":"Short label","description":"One tradeoff sentence","statHints":{"Economy":5,"Military":-2,"Diplomacy":-3,"Stability":5,"GlobalPrestige":-2}},{"id":"a4","label":"Short label","description":"One tradeoff sentence","statHints":{"Economy":-5,"Military":8,"Diplomacy":-8,"Stability":-5,"GlobalPrestige":-3}}]}`;
  ...
  async function handleAction(action) {
    ...
    try {
      const gamePrompt=`Geopolitical simulation — turn ${turn}.
${buildCtx()}
Scenario: ${scenario.title}. Situation: ${situation}
Player action: "${action.label}" — ${action.description}
Stats: ${JSON.stringify(stats)} | Strife: ${strife}/100
...
statChanges -20 to +20. strifeChange -10 to +20. If stat hits 5/95 or strife 90+, gameOver:true.`;
  ...
  async function generateNewspaper(c, s, actionLabel, reactionTxt, turnNum, newStats) {
    const allies=getCountryAlliances(c).map(a=>a.name).join(", ");
    const prompt=`Fictional newspaper front pages for a geopolitical game.
Country: ${c.name} (${c.power}). Scenario: ${s.title}. Action: "${actionLabel}". World reaction: ${reactionTxt}. Turn: ${turnNum}. Stats: Economy ${newStats.Economy}, Military ${newStats.Military}, Diplomacy ${newStats.Diplomacy}, Stability ${newStats.Stability}. Alliances: ${allies||"none"}.
Generate 3 front pages with different editorial biases (Western liberal, Eastern statist, Global South/regional). Same events, very different spin.
Return ONLY valid JSON:
{"papers":[{"outletId":"herald","breakingTag":"BREAKING NEWS","headline":"Max 12 words, Western liberal perspective","subHeadline":"One sentence deck","body":"2-3 sentence article body referencing specific countries/actors","pullQuote":{"text":"Short quote max 15 words","attribution":"Name, Title"},"secondaryStories":[{"section":"WORLD","headline":"Headline","snippet":"One sentence."},{"section":"ECONOMY","headline":"Headline","snippet":"One sentence."},{"section":"OPINION","headline":"Op-ed teaser","snippet":"One sentence."}],"opEd":{"author":"Analyst name","authorIcon":"🎓","title":"Op-ed title","excerpt":"2 sentences commentary."},"ticker":["Item 1","Item 2","Item 3","Item 4"]},{"outletId":"global_t","breakingTag":"EXCLUSIVE","headline":"Eastern/statist framing max 12 words","subHeadline":"One sentence","body":"2-3 sentences emphasising sovereignty or Western hypocrisy","pullQuote":{"text":"Quote","attribution":"Name, Title"},"secondaryStories":[{"section":"GEOPOLITICS","headline":"Headline","snippet":"One sentence."},{"section":"TRADE","headline":"Headline","snippet":"One sentence."}],"opEd":{"author":"Analyst","authorIcon":"🌏","title":"Title","excerpt":"2 sentences praising multipolarity."},"ticker":["Item 1","Item 2","Item 3"]},{"outletId":"al_watan","breakingTag":"URGENT","headline":"Global South perspective max 12 words","subHeadline":"One sentence","body":"2-3 sentences on regional/humanitarian impact","pullQuote":{"text":"Quote","attribution":"Name, Title"},"secondaryStories":[{"section":"REGIONAL","headline":"Headline","snippet":"One sentence."},{"section":"ANALYSIS","headline":"Headline","snippet":"One sentence."}],"opEd":{"author":"Analyst","authorIcon":"🌍","title":"Title","excerpt":"2 sentences on regional consequences."},"ticker":["Item 1","Item 2","Item 3"]}]}`;
        ...
        <div>
          ...
          {reactionText && (
            <div style={{background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderLeft:"3px solid var(--color-border-secondary)",borderRadius:"var(--border-radius-lg)",padding:"1rem 1.25rem",marginBottom:12,animation:"fadeUp 0.3s ease"}}>
              ...
              <p style={{fontSize:14,lineHeight:1.75,margin:"0 0 0",whiteSpace:"pre-wrap",color:"var(--color-text-primary)"}}>
                <TypewriterText text={reactionText} speed={11} />
              </p>
            </div>
          )}

          {/* STAT DELTAS — shown after reaction */}
          ...
          {/* NEWSPAPER — shown inline after reaction */}
          {reactionText && (
            <div style={{marginBottom:12}}>
          ...
          {reactionText && pendingNext && !loading && (
            <div style={{marginBottom:12,animation:"fadeUp 0.4s ease 0.3s both"}}>
              <button onClick={handleContinue}
                style={{width:"100%",padding:"12px",borderRadius:"var(--border-radius-lg)",border:"0.5px solid var(--color-border-primary)",background:"var(--color-background-primary)",cursor:"pointer",fontSize:13,fontWeight:500,color:"var(--color-text-primary)",fontFamily:"var(--font-sans)",transition:"background 0.15s"}}
        ...
        <div style={{position:"sticky",top:"1rem"}}>
          ...
          <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden",marginBottom:10}}>
            <div style={{display:"flex",borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
              {["alliances","actors","diplo"].map(t=>(
                <button key={t} onClick={()=>setSideTab(t)}
                  style={{flex:1,fontSize:10,padding:"7px 4px",background:"none",border:"none",borderBottom:`2px solid ${sideTab===t?"var(--color-text-primary)":"transparent"}`,cursor:"pointer",color:sideTab===t?"var(--color-text-primary)":"var(--color-text-tertiary)",fontFamily:"var(--font-sans)",transition:"border-color 0.15s",letterSpacing:"0.04em"}}>
                  {t==="alliances"?"Alliances":t==="actors"?"Threats":"Diplo"}
                </button>
              ))}
            </div>
            <div style={{padding:"10px 12px",maxHeight:260,overflowY:"auto"}}>
              ...
              {sideTab==="actors" && Object.entries(NON_STATE_ACTORS).map(([k,a])=>{
          ...
          {allEditions.length>0 && (
            <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"10px 12px"}}>
              ...
              {allEditions.map((ed,i)=>{
                ...
                return (
                  <div key={i} style={{marginBottom:7,paddingLeft:8,borderLeft:`2px solid ${o.accentColor}`,cursor:"default"}}>
                    <div style={{fontSize:9,color:o.accentColor,fontFamily:"Georgia,serif",fontWeight:700,marginBottom:2}}>T{ed.turn} · {o.name}</div>
                    <div style={{fontSize:10,fontFamily:"Georgia,serif",color:"#1a1a1a",lineHeight:1.35}}>{p.headline}</div>
                  </div>
                );
              })}
            </div>
